...
export class DoctorLookup {
  ...
  byMedicalIssue(medicalIssue){
    ...
    .then(function(response){
      for (var i = 0; i < response.data.length; i++) {
  			$('#doctorResultsByIssue').append(`<li>Name: ${response.data[i].profile.first_name} ${response.data[i].profile.last_name}<br> Address: ${response.data[i].practices[0].visit_address.city},${response.data[i].practices[0].visit_address.state} <br>Phone Number: ${response.data[i].practices[1].phones[0].number}<br>Accepting New Patients: ${response.data[i].practices[0].accepts_new_patients}<br>Website: ${response.data[i].practices[0].website}</li>`);
		   }
    }).fail(function(error){
      $('.errors').text(`There was an error processing your request: ${error.responseText}. Please try again.`);
    });
